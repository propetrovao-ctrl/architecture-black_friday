...
async def root():
    ...
    if topology_type == "Sharded":
        ...
        for shard in shards_list.get("shards", []):
            ...
            # Получаем информацию о количестве реплик для каждого шарда
            try:
                # Парсим host строку для получения реплик
                # Формат: "shard1/shard1-1:27018,shard1-2:27018,shard1-3:27018"
                if "/" in shard_host:
                    replicas_str = shard_host.split("/")[1]
                    replicas = replicas_str.split(",")
                    shard_replicas[shard_id] = len(replicas)
                else:
                    shard_replicas[shard_id] = 1
            except Exception as e:
                logger.warning(f"Could not parse replicas for {shard_id}: {e}")
                shard_replicas[shard_id] = 1
    ...
    return {
        ...
        "shards": shards,
        "shard_replicas": shard_replicas if shard_replicas else None,
